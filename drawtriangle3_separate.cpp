...
int main()
{
	...
	Shader ourShader1("shader.vs", "shader1.fs"); // you can name your shader files however you like
	Shader ourShader2("shader.vs", "shader2.fs"); // you can name your shader files however you like
	...
	while (!glfwWindowShouldClose(window)) // glfwWindowShouldClose函数在我们每次循环的开始前检查一次GLFW是否被要求退出，如果是的话该函数返回true然后渲染循环便结束了，之后为我们就可以关闭应用程序了。
	{
		...
		ourShader1.use(); // 我们可以调用glUseProgram函数，用刚创建的程序对象作为它的参数，以激活这个程序对象;
		...
		// draw our first triangle
		ourShader2.use(); // 我们可以调用glUseProgram函数，用刚创建的程序对象作为它的参数，以激活这个程序对象;
